...
class _MapPageState extends State<MapPage> {
  ...
  List<Map<String, dynamic>> get _filteredVenues {
    return _selectedSport == 'All'
        ? chicagoVenues
        : chicagoVenues.where((venue) =>
            (venue['sports'] as List).contains(_selectedSport)).toList();
  }
  ...
  Set<Marker> _getVenueMarkers() {
    return _filteredVenues.map((venue) => Marker(
          ...
          infoWindow: InfoWindow(
            title: venue['name'],
            snippet: '${(venue['sports'] as List).join(', ')} — ${venue['hours']}',
          ),
        ...
        children: [
          ...
          DropdownButton<Map<String, dynamic>>(
            ...
            items: _filteredVenues
                .map((venue) => DropdownMenuItem<Map<String, dynamic>>(
                      value: venue,
                      child: Text('${venue['name']} - ${(venue['sports'] as List).join(', ')}'),
                    ))
                .toList(),
            onChanged: (venue) {
              if (venue != null) {
                showDialog(
                  ...
                  builder: (_) => AlertDialog(
                    title: Text(venue['name']),
                    content: Text(
                      'Sports: ${(venue['sports'] as List).join(', ')}\n'
                      'Hours: ${venue['hours']}\n'
                      'Activity Level: ${venue['activity']}★',
                    ),
                  ),
